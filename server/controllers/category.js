...
export const createCategory = async (req, res) => {
    ...
    switch(true){
      ...
      case !sub_title:
		...
		const isSubTitleExists = await category.findOne({$and:[{sub_title}, {name}]});
		if(isSubTitleExists){
		...
		const newCategory = await new category({title, sub_title, name, slug}).save();
		res.status(200).send({
			success: true, 
...
export const updateCategory = async (req, res) => {
	try{
		const { title, sub_title, name }= req.body.name;
    ...
    switch(true){
      ...
      case name:
		...
		const slug = slugify(name);
		const newCategory = await category.findByIdAndUpdate(id, {name, title,  sub_title, slug}, {new: true});
		res.status(200).send({
			success: true, 
