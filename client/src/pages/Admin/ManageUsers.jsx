  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(
        ...
        { name: name, email: email, phone: phone, privilege: 3 },
      );
      if (res.data?.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_API}/api/v1/admin/${eleId}`,
      );
      if (res.data.success) {
                  ...
                  {users?.map((ele) => (
                    <tr>
                      ...
                      <td className="pt-2 ps-5">
                        <span
                          className="badge bg-primary rounded-pill ps-3 pe-3 pt-2 pb-2"
                          onClick={() => {
                            setVisible(true);
                            setName(ele.name);
                            setEmail(ele.email);
                        ...
                        <span
                          className="badge bg-secondary ms-2 rounded-pill ps-3 pe-3 pt-2 pb-2"
                          onClick={() => {
                            setPrompt(true);
                            setEleId(ele._id);
                            setName(ele.name);
                        ...
                        {auth?.user?.privilege === 0
                          ? (
                            <>
                              <span
                                className="badge bg-danger ms-2 rounded-pill ps-3 pe-3 pt-2 pb-2"
                                onClick={() => {
                                  setAdminPrompt(true);
                                  setSelected(ele);
                                  setName(ele.name);
